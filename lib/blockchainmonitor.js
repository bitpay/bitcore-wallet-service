...
BlockchainMonitor.prototype._storeAndBroadcastNotification = function(notification, cb) {
  ...
  self.storage.storeNotification(notification.walletId, notification, function(err, response) {
    if (err) {
      log.error(err);
    }
    // We only want broadcast the message once to the wallet listeners
