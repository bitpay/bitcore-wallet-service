...
BlockchainMonitor.prototype._handleNewBlockchainTip = function(network, hash, cb) {
  ...
  self.storage.getTip(network, function(err, tip) {
    if (err) return cb(err);
    ...
    log.debug('', 'Last processed tip for %s was: %s on ', network, tip.hashes[0], (new Date(1000 * tip.updateOn)).toString());
    ...
    self._fetchAndProcessBlock(network, hash, function(err, block) {
      ...
      if (_.indexOf(tip.hashes, header.prevHash) > 0) {
        log.info('', 'Prevhash does not match stored tip hashes... Processing: ' + header.prevHash);
        self._processBlockchainSince(network, header.prevHash, cb);
      } else {

        tip.hashes.unshift(hash);
        var hashes = tip.hashes.splice(0, Defaults.MAX_REORG_DEEPTH);
        log.debug('', 'Storing new tip:' + hashes);
        self.storage.updateTip(network, hashes, cb);
      }
    });
  });
