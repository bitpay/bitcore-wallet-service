...
var RBF_CONFIRMATION_WAITING_TIME = 48; // Time (in hours) to wait for RBF transactions to confirm before sending notification
...
BlockchainMonitor.prototype._processIncomingTxQueue = function(cb) {
  ...
  // Remove old items
  var threshold = moment().subtract(RBF_CONFIRMATION_WAITING_TIME, 'hours');
  _incomingTxQueue = _.reject(_incomingTxQueue, function(item) {
    return moment(item.createdOn).isBefore(threshold);
  });
  ...
  async.each(_incomingTxQueue, function(tx, next) {
    if (tx.isRBF) {
      self.explorers[tx.network].getTransaction(tx.txid, function(err, transaction) {
        if (err) {
          log.debug('Could not verify transaction ' + tx.txid, err);
          return next();
        }
