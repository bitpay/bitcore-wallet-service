...
BlockchainMonitor.prototype._handleIncomingPayments = function(coin, network, data) {
  ...
  async.each(outs, function(out, next) {
    ...
    self.storage.fetchAddressByCoin(coin, out.address, function(err, address) {
      if (err) {
        log.error('', 'Could not fetch address %s from the db', out.address);
        return next(err);
      }
      ...
      // handle tx and balance cache
      self.storage.softResetTxHistoryCache(walletId, function() {
        self.storage.updateAddressesWithUtxo(walletId, [address], {partial:true}, function(err) {
          if (err) {
            log.error('Could not fetch address from the db', err);
          }
          log.info('Keeping addr alive: ' + address.address);
        });
      });
