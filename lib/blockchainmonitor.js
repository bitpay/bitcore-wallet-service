...
BlockchainMonitor.prototype.start = function(opts, cb) {
  ...
  async.parallel([
    ...
    function(done) {
      ...
      // self.messageBroker.onMessage(_.bind(self._handleIncomingTx, self));
      done();
    },
    function(done) {
...
BlockchainMonitor.prototype._processIncomingTxQueue = function(cb) {
  ...
  var queue = _.cloneDeep(_incomingTxQueue);
  async.each(queue, function(record, next) {
    var notification = Notification.create({
      type: 'NewIncomingTx',
      data: {
        txid: record.txid,
    ...
    self._updateActiveAddresses(record.walletId, record.address, function() {
      self._storeAndBroadcastNotification(notification, function(err) {
        _incomingTxQueue = _.without(_incomingTxQueue, record);
        return next(err);
      });
    });
  }, cb);
...
BlockchainMonitor.prototype._handleTxOuts = function(data, cb) {
  ...
  async.each(outs, function(out, next) {
    ...
    self._processIncomingTxQueue(cb);
  });
};
