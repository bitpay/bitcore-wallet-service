...
Storage.prototype._createIndexes = function() {
  ...
  // this.db.collection(collections.USER_STATS).createIndex({
  //   time: 1,
  //   userToken: 1,
  // });
...
Storage.prototype.storeUserStats = function(stats, cb) {
  if (!stats) return cb();
  stats.time = Date.now();
  this.db.collection(collections.USER_STATS).insert(stats, {
    w: 1,
  }, cb);
};
