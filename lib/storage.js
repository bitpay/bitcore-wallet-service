...
Storage.prototype._createIndexes = function() {
  ...
  // this.db.collection(collections.USAGE_STATS).createIndex({
  //   time: 1,
  //   user: 1,
  // });
...
Storage.prototype.storeUsageStats = function(stats, cb) {
  if (!stats) return cb();

  this.db.collection(collections.USAGE_STATS).insert(stats, {
    w: 1,
  }, function(err) {
    if (err) return cb(err);
    delete stats._id;
