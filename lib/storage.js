...
Storage.prototype.updateBlockchainTip = function(network, hashes, cb) {
...
Storage.prototype.updateLastUsedOn = function(addresses, time, cb) {
  var self = this;
  ...
  async.eachLimit(addresses, 10, function(address, next) {
    ...
    self.db.collection(collections.ADDRESSES).update({
      ...
      $set: {
        lastUsedOn: time || Math.floor(Date.now() / 1000),
      },
    }, {
      w: 1,
    }, next);
...
Storage.prototype.fetchRecentAddresses = function(walletId, minUsedOn, cb) {
  var self = this;
