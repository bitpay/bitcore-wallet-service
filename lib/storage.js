...
var BIP44_GAP = 20;
Storage.prototype.fetchNewAddresses = function(walletId, cb) {
  var self = this;
  ...
  this.db.collection(collections.ADDRESSES).find({
    ...
    _id: -1,
  }).limit(BIP44_GAP).toArray(function(err, result1) {
    if (err) return cb(err);
    ...
    self.db.collection(collections.ADDRESSES).find({
      ...
      _id: -1
    }).limit(BIP44_GAP).toArray(function(err, result2) {
      if (err) return cb(err);

      result2 = result2 || [];

      var result = _.compact(result1.concat(result2));
      if (!result) return cb();

      var addresses = _.map(result, function(address) {
        return Model.Address.fromObj(address);
      });
      return cb(null, addresses, result1.length, result2.length);
    });
  });
};
...
Storage.prototype.storeAddressCacheStatus = function(walletId, cacheStatus, cb) {
  var self = this;
  self.db.collection(collections.CACHE).update( { 
    walletId: walletId,
    type: 'addressCacheStatus',
    key: null,
  }, {
    "$set":
    { 
...
Storage.prototype.getAddressCacheStatus = function(walletId, cb) {
  var self = this;
...
Storage.prototype.updateAddressesWithUtxo = function(walletId, addresses, opts, cb) {
  var self = this;

  var now = Date.now();
  var mainLimit = now - Defaults.DA_MAIN_MONITORING_DAYS * 86400 * 1000;
  var changeLimit = now - Defaults.DA_CHANGE_MONITORING_DAYS * 86400 * 1000;

  if (_.isEmpty(addresses))  
    addresses = [];

  $.checkState(_.isObject(addresses[0]));
  self.db.collection(collections.CACHE).findOne({
    walletId: walletId,
    type: 'addressesWithUtxo',
    key: null, 
  }, function(err, prev) {
    if (err) return cb(err);
    ...
    // remove old
    if (!opts.partial) {
      prev = _.filter(prev, function(x) {
        if (!x.isChange && x.ts > mainLimit) 
          return true;
        if (x.isChange && x.ts > changeLimit) 
          return true;
        return false;
      });    
    }
    ...
    var now = Date.now();
    var newAddr = _.map(addresses, function(x) {
      return {
        address: x.address,
        isChange: x.isChange,
        ts: now,
    ...
    var finalAddr = _.uniq(newAddr.concat(prev), 'address');
    self.db.collection(collections.CACHE).update({
      walletId: walletId,
      type: 'addressesWithUtxo',
      key: null,
      ...
      return cb(err, finalAddr.length);
    });
  });
};


Storage.prototype._toAddressObj = function(walletId, addresses, cb)  {
  var self = this;
...
Storage.prototype.fetchAddressesWithUtxo = function(walletId, cb) {
  var self = this;
...
console.log('[storage.js.766]'); //TODO
    return self._toAddressObj(walletId,  _.map(result.addresses,'address'), cb);
  });
};
...
Storage.prototype._getAddressesFromHistoryCache = function(walletId, nr, cb) {
  var self = this;
  ...
  self.db.collection(collections.CACHE).find({
    ...
    id: -1,
  }).limit(nr).toArray(function(err, result) {
    if (err) return cb(err);
    if (!result) return cb();
    var txs = _.map(result, 'tx');
    var addr = [];
    _.each(txs, function(x) {
      addr = addr.concat(_.map(x.tx.inputs,'address'));
      addr = addr.concat(_.map(x.tx.outputs,'address'));
    });
    return cb(null, _.uniq(addr));
  });
};

Storage.prototype.getAddressesFromHistoryCache = function(walletId, nr, cb) {
  var self = this;

  self._getAddressesFromHistoryCache(walletId, nr, function(err, addresses) {
    if (err) return cb(err); 
    return self._toAddressObj(walletId, addresses, cb);
  });
};
...
Storage.prototype._addressHash = function(addresses) {
  // This could be slow... but order change depending the source of the addr
  var all = _.sortBy(addresses).join();
  return Bitcore.crypto.Hash.ripemd160(new Buffer(all)).toString('hex');
};

Storage.prototype.checkAndUseUtxoCache = function(walletId, addresses, duration, cb) {
  var self = this;
  ...
  self.db.collection(collections.CACHE).findOne({
    ...
    if (validFor > 0)  {
      log.debug('','Using UTXO Cache valid for %d ms more', validFor); 
      cb(null, ret.result);
      return true;
    }
    cb();
...
Storage.prototype.storeUtxoCache = function (walletId, addresses, utxo, cb) {
