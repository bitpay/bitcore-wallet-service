...
WalletService.prototype._getAliveAddresses = function(cb) {
  ...
  self._checkAndUpdateAddressCacheStatus(function(err, da_active ) {
    ...
    async.parallel([
      ...
      // Address from recent TXs
      function(next) {
        // Get all types of addresses from main ts.
        self.storage.getAddressesFromHistoryCache(self.walletId, Defaults.DA_TX_TO_SCAN_ADDR, function(err, addr) {
          if (!addr || !addr.length) return next();
      ...
      var len = _.map(res, function(x) { 
        if (!x) return 0; 
        return x.length;
      });

      // TODO: warning with lodash 4: replace with 'uniqBy' keys: _.contains _.all
      var f = _.uniq(_.compact(_.flatten(res)), 'address');
      self.logi('Total addresses alive: prev: %d new: %d recent: %d: total %d ', len[0], len[1], len[2],  f.length);

      if (_.isEmpty(f))
        return cb();

      return cb(null, f, len[0], len[1], len[2]);
    });
  });
};
 ...
 */
// noLock cannot be passed from the outside
WalletService.prototype.getSendMaxInfo = function(opts, cb, noLock) {
  var self = this;
