...
WalletService.prototype._notify = function(type, data, opts, cb) {
  ...
  this.storage.storeNotification(walletId, notification, function(err, response) {
    if (err) {
      log.error(err);
    }
    if (response && response.result.upserted) {
...
WalletService.prototype.startScan = function(opts, cb) {
  ...
  function scanFinished(err, wallet) {
    var data = {
      timestamp: Date.now(),
      walletId: wallet.id,
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    setTimeout(function() {
      self.scan(opts, function(err) {
        scanFinished(err, wallet);
      });
    }, 100);
