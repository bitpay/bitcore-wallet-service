...
WalletService.prototype._processBroadcast = function(txp, opts, cb) {
  ...
  // rebroadcast?
  if (txp.status == 'broadcasted' && opts.force) {

    // no notification, no storage
    return self.storage.softResetTxHistoryCache(self.walletId, function(err) {
      return cb(err, txp);
    });
  }
 ...
 * @param {Object} opts
 * @param {string} opts.txProposalId - The identifier of the transaction.
 * @param {string} opts.force - rebroadcast
 */
WalletService.prototype.broadcastTx = function(opts, cb) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    self.getTx({
      ...
      if (opts.force) {
        if (txp.status != 'accepted' && txp.status != 'broadcasted')  {
...
// TODO: Rebroadcast unconfirmed TXPs??
