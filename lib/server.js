 ...
 * @returns {Address[]}
 */
WalletService.prototype.getAddressFromWallet = function(opts, cb) {
  var self = this;

  if (!checkRequired(opts, ['address', 'walletId'], cb)) return;

  self.storage.fetchAddressFromWallet(opts.walletId, opts.address, function(err, addresses) {
    if (err) return cb(err);
