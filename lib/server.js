...
WalletService.prototype._syncWithAddressService = function(cb) {
  var self = this;

  function registerWallet(cb) {
    self.storage.fetchAddressStrs(self.walletId, function(err, addresses) {
      if (err) return cb(err);

      self.addressService.createGroup(addresses, function(err, groupId) {
        if (err) return cb(err);

        self.storage.updateWalletForAddressService(self.walletId, groupId, addresses, cb);
      });
    });
  };
  ...
  function addUnregisteredAddresses(wallet, cb) {
    ...
    self.storage.fetchUnregisteredAddressStrs(self.walletId, function(err, addresses) {
      if (err) return cb(err);
      ...
      as.addAddresses(wallet.addressServiceId, addresses, function(err) {
        ...
        self.storage.updateAddressesAddressServiceId(self.walletId, addresses, cb);
      });
    });
  };
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    if (!wallet.addressServiceId) return registerWallet(cb);
    addUnregisteredAddresses(wallet, cb);
  });
};
