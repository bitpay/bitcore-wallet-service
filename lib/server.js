    ...
    async.parallel([
      function(next) {
        ...
        self.storage.fetchAddressesWithUtxo(self.walletId, function(err, res) {
          ...
          self.logi(' * Adding addresses with recent balance: %d', res.length);
      ...
      function(next) {
        ...
        self.storage.fetchNewAddresses(self.walletId, function(err, recent, nrMain, nrChange) {
          ...
          self.logi(' * Adding recent new addresses: m:%s + c:%d', nrMain, nrChange);
      ...
      function(next) {
        ...
        self.storage.getAddressesFromHistoryCache(self.walletId, Defaults.DA_TX_TO_SCAN_ADDR, function(err, addr) {
          ...
          self.logi(' * Adding recent tx addresses', addr.length);
          return next(null, recent);
        });
      },
    ], function(err, res) {
