 ...
 * @param {Object} opts - An object identified by the 'user' field.
 */
WalletService.prototype.saveUsageStats = function(opts, cb) {
  var self = this;

  if (!checkRequired(opts, ['user'], cb)) return;

  opts.time = Date.now();
  self.storage.storeUsageStats(opts, function(err) {
    if (err) return cb(err);
    return cb(null, opts);
  });
};
