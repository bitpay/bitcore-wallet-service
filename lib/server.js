...
WalletService.prototype._notify = function(type, data, opts, cb) {
  ...
  this.storage.storeNotification(walletId, notification, function() {
    ...
    console.log('debug storeNotification callback', type, data);
    return cb();
  });
};
...
WalletService.prototype.joinWallet = function(opts, cb) {
  console.log('debug joinWallet 1');
  var self = this;
  ...
  console.log('debug joinWallet 2');
  self._runLocked(cb, function(cb) {
    console.log('debug joinWallet 3 '+opts.walletId);
    self.storage.fetchWallet(opts.walletId, function(err, wallet) {
      console.log('debug joinWallet 4');
      ...
      var hash = WalletUtils.getCopayerHash(opts.name, opts.xPubKey, opts.requestPubKey);
      if (!self._verifySignature(hash, opts.copayerSignature, wallet.pubKey)) {
        console.log('debug joinWallet 5');
        return cb(new ClientError());
      }
      ...
      console.log('debug joinWallet 6');
      ...
      console.log('debug joinWallet 7');

      self.storage.fetchCopayerLookup(copayer.id, function(err, res) {
        console.log('debug joinWallet 8');
        if (err) return cb(err);
        if (res)
        ...
        self.storage.storeWalletAndUpdateCopayersLookup(wallet, function(err) {
          console.log('debug joinWallet 9');
          if (err) return cb(err);
          ...
          async.series([
            ...
            function(next) {
              console.log('debug joinWallet 10');
            ...
            function(next) {
              console.log('debug joinWallet 11');
            ...
            console.log('debug joinWallet 12');
            return cb(null, {
              copayerId: copayer.id,
              wallet: wallet
            });
