 ...
 * @param {Object} opts - An object identified by userToken
 */
WalletService.prototype.saveUserStats = function(opts, cb) {
  var self = this;

  if (!checkRequired(opts, ['userToken'], cb)) return;

  opts.time = Date.now();
  self.storage.storeUserStats(opts, function(err) {
    if (err) return cb(err);
    return cb(null, opts);
  });
};
