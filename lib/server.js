...
WalletService.prototype._getBalanceOneStep = function(opts, cb) {
  ...
  function updateCache(balance, done) {
    async.series([
  ...
  function updateStats(balance, done) {
    self.getWallet({}, function(err, wallet) {
      if (err) return cb(err);
      ...
      var stats = {
        ...
        addressesWithBalance: balance.byAddress.length,
      }
      self.storage.storeStat('balance', self.walletId, stats, done);
    });
  };
  ...
  self.storage.fetchAddresses(self.walletId, function(err, addresses) {
    ...
    self._getBalanceFromAddresses(addresses, function(err, balance) {
      ...
      async.parallel([
          ...
          function(next) {
            updateCache(balance, next);
          },
          function(next) {
            updateStats(balance, next);
          },
        ],
        function(err) {
          return cb(null, balance);
