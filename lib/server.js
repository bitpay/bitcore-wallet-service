...
WalletService.initialize = function(opts, cb) {
  ...
  addressServiceOpts = opts.addressServiceOpts;
...
WalletService.prototype._syncWithAddressService = function(wallet, cb) {
  var self = this;

  if (_.isFunction(wallet)) {
    cb = wallet;
    wallet = null;
  }
  cb = cb || function() {};

  function registerWallet(wallet, cb) {
    var as = self._getAddressService(wallet.network);

    as.createGroup(function(err, addressServiceId) {
      if (err) return cb(err);

      self.storage.updateWalletAddressServiceId(wallet.id, addressServiceId, function(err) {
        if (err) return cb(err);
        wallet.addressServiceId = addressServiceId;
        return cb();
      });
    });
  };

  function addUnregisteredAddresses(wallet, cb) {
    self.storage.fetchUnregisteredAddressStrs(wallet.id, function(err, addresses) {
      if (err) return cb(err);
      if (_.isEmpty(addresses)) return cb();

      var as = self._getAddressService(wallet.network);
      as.addAddresses(wallet.addressServiceId, addresses, function(err) {
        if (err) return cb(err);

        self.storage.updateAddressesAddressServiceId(wallet.id, addresses, cb);
      });
    });
  };
  ...
  async.series([
    ...
    function(next) {
      if (wallet.addressServiceId) return next();
      registerWallet(wallet, next);
    },
    function(next) {
      addUnregisteredAddresses(wallet, next);
    },
  ], cb);
};
  ...
  async.series([
    ...
    function(next) {
      ...
      self.storage.fetchWallet(opts.id, function(err, wallet) {
        if (wallet) return next(Errors.WALLET_ALREADY_EXISTS);
        return next(err);
      });
    },
    function(next) {
    ...
    function(next) {
      if (newWallet) self._syncWithAddressService(newWallet); // Do not wait for callback
      next();
    },
  ], function(err) {
...
WalletService.prototype._getAddressService = function(network) {
  if (!this.addressService) {
    var opts = {};
    if (this.addressServiceOpts && this.addressServiceOpts[network]) {
      opts = this.addressServiceOpts[network];
    }
    opts.network = network;
    opts.userAgent = WalletService.getServiceVersion();
    this.addressService = new AddressService(opts);
  }
...
WalletService.prototype._getUtxosForCurrentWallet = function(cb) {
  ...
  function getUtxos(wallet, cb) {
    $.checkState(wallet.addressServiceId);

    var as = self._getAddressService(wallet.network);
    as.getUtxos(wallet.addressServiceId, function(err, utxos) {
      if (err) return cb(err);

      var utxos = _.map(utxos, function(utxo) {
        var u = _.pick(utxo, ['txid', 'vout', 'address', 'scriptPubKey', 'satoshis', 'confirmations']);
        u.confirmations = u.confirmations || 0;
        u.locked = false;
        u.satoshis = _.isNumber(u.satoshis) ? +u.satoshis : Utils.strip(utxo.amount * 1e8);
        u.vout = _.isNumber(u.vout) ? u.vout : utxo.outputIndex;
        return u;
      });
  ...
  self._syncWithAddressService(function(err) {
    if (err) return cb(err);

    self.getWallet({}, function(err, wallet) {
      if (err) return cb(err);

      getUtxos(wallet, function(err, allUtxos) {
        if (err) return cb(err);

        if (allUtxos.length == 0) return cb(null, []);
        var utxoIndex = _.indexBy(allUtxos, utxoKey);
        ...
        async.series([
          ...
          function(next) {
            ...
            var addressStrs = _.uniq(_.pluck(allUtxos, 'address'));
            self.storage.fetchAddresses(self.walletId, addressStrs, function(err, addresses) {
...
WalletService.prototype.getUtxos = function(opts, cb) {
  ...
  if (opts.addresses) {
    return cb(new ClientError(Errors.codes.DEPRECATED, 'Providing a list of addresses is no longer supported.'));
  }
...
WalletService.prototype.getTxHistory = function(opts, cb) {
  ...
  function decorate(wallet, txs, addresses, proposals, notes) {
    ...
    return _.map(txs, function(tx) {
      ...
      if (tx.outputs.length || tx.inputs.length) {
        ...
        } else if (action == 'received' || action == 'moved') {
          var firstInternalOutput = _.find(outputs, {
            isMine: true
          });
          addressTo = firstInternalOutput ? firstInternalOutput.address : 'N/A';
        }
      } else {
        action = 'invalid';
        amount = 0;
  ...
  function getNormalizedTxs(wallet, from, to, cb) {
    var as = self._getAddressService(wallet.network);
    as.getTransactions(wallet.addressServiceId, from, to, function(err, rawTxs) {
      if (err) return cb(err);

      var txs = self._normalizeTxHistory(rawTxs);
      return cb(null, txs);
    });
  };

  function extractAddresses(txs) {
    return _.compact(_.flatten(_.map(txs, function(tx) {
      var fromInputs = _.pluck(tx.inputs, 'address');
      var fromOutputs = _.pluck(tx.outputs, 'address');
      return fromInputs.concat(fromOutputs);
    })));
  };

  function extractTxids(txs) {
    return _.pluck(txs, 'txid');
  };

  self.getWallet({}, function(err, wallet) {
    if (err) return cb(err);

    self._syncWithAddressService(wallet, function(err) {
      if (err) return cb(err);
      ...
      async.waterfall([
        ...
        function(next) {
          getNormalizedTxs(wallet, from, to, next);
        },
        function(txs, next) {
          var addressStrs = extractAddresses(txs);
          var txids = extractTxids(txs);

          async.parallel([

            function(done) {
              self.storage.fetchAddresses(self.walletId, addressStrs, done);
            },
            function(done) {
              self.storage.fetchTxsByTxid(self.walletId, txids, done);
            },
            function(done) {
              // TODO: fetch notes only for relevant txs
              self.storage.fetchTxNotes(self.walletId, {}, done);
            },
          ], function(err, res) {
            return next(err, {
              addresses: res[0],
        ...
        var finalTxs = decorate(wallet, res.txs, res.addresses, res.txps, res.notes);
