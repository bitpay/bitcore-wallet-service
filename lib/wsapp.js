...
WsApp.prototype._handleNotification = function(notification) {
  ...
  namespace.emit('multimessage', {to: to, message: 'notification', payload: notification});
};
  ...
  WalletService.getInstanceWithAuth(
    ...
    function(err, service) {
      ...
      if (service.walletId in self.authorizedWallets)
        arr = self.authorizedWallets[service.walletId];
      else
        arr = self.authorizedWallets[service.walletId] = [];
      if (arr.indexOf(connectionId) < 0)
        arr.push(connectionId);
