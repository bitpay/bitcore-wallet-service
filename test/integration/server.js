...
describe('Wallet service', function() {
  ...
  describe('#getBalance alive&dead addresses ', function() {
          ...
          server.getWallet({}, function(err, wallet) {
            ...
            helpers.createAddresses(server, wallet, 25, 0, function(mainAddresses, changeAddress) {
              ...
              server.getBalance({}, function(err, balance, fromCache, nr) {
                ...
                // There should be 2 active addresses with balance + last 20 main
                nr.should.equal(2 + 20); 
                done();
              });
            });
          });
    ...
    it('should stop monitoring addresses without balance after while', function(done) {
      helpers.stubUtxos(server, wallet, [1, 'u2', 3], function() {
        server.getBalance({}, function(err, balance, fromCache, nr) {
          nr.should.equal(2); 
          clock.tick(( Defaults.DA_MAIN_MONITORING_DAYS +1) * 86400 * 1000);
          // nr are the addresses that stayed in "with recent balance"
          //
          should.not.exist(err);
          server.getWallet({}, function(err, wallet) {
            helpers.stubAddressActivityTrue();
            helpers.createAddresses(server, wallet, 25, 0, function() {
              helpers.stubUtxos(server, wallet, [1], function() {
                server.getBalance({}, function(err, balance, fromCache, nr) {
                  // Query: 22, then updates to 20
                  nr.should.equal(22); 

                  server.getBalance({}, function(err, balance, fromCache, nr) {
                  should.exist(balance);
                  balance.totalAmount.should.equal(helpers.toSatoshi(1));

                  should.not.exist(fromCache);

                  // There should be + last 20 main
                  nr.should.equal(20); 
                  done();
                  });
                });
    ...
    it('should update alive addresses, on new utxos while the others stay', function(done) {
      helpers.stubUtxos(server, wallet, [1, 'u2', 3], function() {
        server.getBalance({}, function(err, balance, fromCache, nr) {
          should.not.exist(err);
          should.not.exist(fromCache);
          ...
          helpers.stubUtxos(server, wallet, [1, 'u2', 2], {keepOld:true}, function() {
            server.getBalance({}, function(err, balance, fromCache, nr) {
              should.not.exist(err);
              should.not.exist(fromCache);
              nr.should.equal(4); 
    ...
    it('should update alive addresses, on new utxos while the other are dead', function(done) {
      helpers.stubUtxos(server, wallet, [1, 2], function() {
        server.getBalance({}, function(err, balance, fromCache, nr) {
          should.not.exist(err);
          should.not.exist(fromCache);
    ...
    it('should update alive addresses, on new utxos while the other are gone (case 2)', function(done) {
      helpers.stubUtxos(server, wallet, [1, 2], function() {
        server.getBalance({}, function(err, balance, fromCache, nr) {
          should.not.exist(err);
          should.not.exist(fromCache);
          ...
          server.getWallet({}, function(err, wallet) {
            // generate GAP addresses
            helpers.createAddresses(server, wallet, 5, 5, function(mainAddresses, changeAddress) {
              server.getBalance({}, function(err, balance, fromCache, nr) {
                should.not.exist(err);
                should.not.exist(fromCache);

                // 1 with balance, 5 new, 5 change new.
                nr.should.equal(11); 
                done();
              });
            });
    ...
    it('should query the entire wallet if there is not prev info', function(done) {
      // generate GAP addresses
      helpers.stubUtxos(server, wallet, [1], function() {
        server.getWallet({}, function(err, wallet) {
          helpers.createAddresses(server, wallet, 25, 5, function(mainAddresses, changeAddress) {
            server.getBalance({}, function(err, balance, fromCache, nr) {
              should.not.exist(err);
              should.not.exist(fromCache);

              // all addresses, 25+5  +1  (stubUtxo)
              nr.should.equal(31); 
              done();
            });
          });
    ...
    it('should update alive addresses, on new change', function(done) {
      helpers.stubUtxos(server, wallet, [1, 'u2', 3], function() {
        server.getBalance({}, function(err, balance, fromCache, nr) {
          should.not.exist(err);
          should.not.exist(fromCache);
  ...
  describe('#getBalance fast cache', function() {
    var server, wallet, clock;
    var _old = Defaults.UTXO_CACHE_ADDRESS_THRESOLD;
    beforeEach(function(done) {
      clock = sinon.useFakeTimers(Date.now(), 'Date');
      Defaults.UTXO_CACHE_ADDRESS_THRESOLD = 0;
    ...
    afterEach(function() {
      clock.restore();
      Defaults.UTXO_CACHE_ADDRESS_THRESOLD = _old;
    });
    ...
    it('should get balance with/without cache', function(done) {
      ...
      helpers.stubUtxos(server, wallet, [1, 'u2', 3], function() {
        server.getBalance({
          ...
          setTimeout(function() {
            server.getBalance({}, function(err, balance, cacheUsed) {
              should.not.exist(err);
